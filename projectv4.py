...
def flatRateTariff(df, flatRate, fixedFee):  # returns a dictionary
    ...
    try:
        energyCost = float(totalKWh) * float(flatRate)
    except Exception:
        energyCost = 0.0
    ...
    breakdown = {"Energy": float(energyCost)}
    return {
        "scheme": "Flat",
        "totalBill": float(energyCost + fixedFee),
        "breakdown": breakdown,
        "fixedFee": float(fixedFee),
...
def touTariff(df, touRates, fixedFee):  # returns a dictionary
    ...
    try:
        ...
        df["timestamp"] = pd.to_datetime("1970-01-01")
    ...
    for _, row in df.iterrows():
        ...
        for period, info in touRates.items():
            try:
                ...
                if s < e:
                    if s <= timestamp < e:
                        breakdown_costs[period] = breakdown_costs.get(period, 0.0) + kWh * rate
                        applied = True
                        break
                else:
    ...
    breakdown_costs["Fixed Fee"] = fixedFee
    total = sum(breakdown_costs.values())

    return {
        "scheme": "TOU",
        "totalBill": float(total),
        "breakdown": {k: float(v) for k, v in breakdown_costs.items()},
        "fixedFee": fixedFee
    }


# Function 3: Tiered Tariff
def tieredTariff(df, tier_limits, tier_rates, fixed_fee):
    ...
    if remaining_usage > 0 and len(tier_rates) > len(tier_limits):
        try:
            rate = float(tier_rates[-1])
            breakdown[f"Tier {len(tier_limits) + 1}"] = remaining_usage * rate
        except Exception:
            breakdown[f"Tier {len(tier_limits) + 1}"] = 0.0
